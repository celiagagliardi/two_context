%% output
% Context Prediction using average dot product of population vectors OR CORRELATION 
% Day 1 FI cells
% 	 avg = 0.6622, sem = 0.0722
% 		 one sample ttest: t(11) = 2.247, p = 0.02305, using FI cells and 12 animals
% 		 signrank test: p = 0.031
% Day 2 FI cells
% 	 avg = 0.6574, sem = 0.0876
% 		 one sample ttest: t(10) = 1.797, p = 0.05125, using FI cells and 11 animals
% 		 signrank test: p = 0.056
% Day 3 FI cells
% 	 avg = 0.7387, sem = 0.0601
% 		 one sample ttest: t(10) = 3.973, p = 0.00132, using FI cells and 11 animals
% 		 signrank test: p = 0.002
% Context Prediction using average dot product of population vectors OR CORRELATION 
% Day 1 FS cells
% 	 avg = 0.8519, sem = 0.0477, using FS cells and 9 animals
% 		 one tail t test: t(8) = 7.382, p = 0.00004
% 		 sign rank test: p = 0.002
% Day 2 FS cells
% 	 avg = 0.8025, sem = 0.0980, using FS cells and 7 animals
% 		 one tail t test: t(6) = 3.086, p = 0.01075
% 		 sign rank test: p = 0.031
% Day 3 FS cells
% 	 avg = 0.9460, sem = 0.0234, using FS cells and 8 animals
% 		 one tail t test: t(7) = 19.086, p = 0.00000
...
%         % normalize maps
%         fi_maps = cellfun(@(x) x./max(x,[], 'all') , fi_maps, 'uniformoutput', false);
%         fs_maps = cellfun(@(x) x./max(x,[], 'all') , fs_maps, 'uniformoutput', false);
...
scatter(1, cell2mat(fi_pred(:,1)), 'ko', 'xjitter', 'rand');
scatter(2, cell2mat(fi_pred(:,2)), 'ro', 'xjitter', 'rand');
scatter(3, cell2mat(fi_pred(:,3)), 'bo', 'xjitter', 'rand');
...
yline(0.5, 'k', 'linewidth', 2);
scatter(1, cell2mat(fs_pred(:,1)), 'ko', 'xjitter', 'rand');
scatter(2, cell2mat(fs_pred(:,2)), 'ro', 'xjitter', 'rand');
scatter(3, cell2mat(fs_pred(:,3)), 'bo', 'xjitter', 'rand');
