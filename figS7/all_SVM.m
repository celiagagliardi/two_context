...
switch cell2analyze
    ...
    case 'FS'
        celltype = baData(baData.isStable == 0,:);
    case 'all'
        celltype = baData;
end

fprintf('Analysis done on %s cells\n', cell2analyze);
...
for d = 1:3
    ...
    for a = 1:length(animals)
       ...
       % if sum(ismember(digClass, 'Corr')) > 1 && sum(ismember(digClass, 'Geo')) > 1
       ...
       % if sum(ismember(contextClass, '1')) > 1 && sum(ismember(contextClass, '2')) > 1
...
xlabel('Day');
title(sprintf('%s cell context prediction', cell2analyze));
