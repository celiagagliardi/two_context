...
for d = 1:3
    ...
    [p] = signrank(fidata, .5, 'tail', 'right');
    fprintf('\tp = %.3f\n', p);
    fprintf('\tFS Heading prediction avg = %.3f, sem = %.3f\n', avg(2,d), sem(2,d));
    [p] = signrank(fsdata, .5, 'tail', 'right');
    fprintf('\tp = %.3f\n', p);
    [h,p, ci, stats] = ttest2(fidata, fsdata);
    fprintf('\t\t FI vs FS data: h = %d, t(%d) = %.3f, p = %.5f\n', h,stats.df, stats.tstat, p);
    % 
end
avg = avg';
sem = sem';
...
t = table(FIcontext(:,1), FIcontext(:,2), FIcontext(:,3), ...
    FScontext(:,1), FScontext(:,2), FScontext(:,3), 'VariableNames', ...
    {'FI context day 1', 'FI context day 2', 'FI context day 3', ...
    'FS context day 1', 'FS context day 2', 'FS context day 3'}, 'RowNames', allanimals);
...
%% make excel sheet
T = cell2table([FIcontext, FScontext, FIheading, FSheading], 'VariableNames', ...
    {'FI_context_prediction_day1','FI_context_prediction_day2','FI_context_prediction_day3', ...
    'FS_context_prediction_day1','FS_context_prediction_day2','FS_context_prediction_day3', ...
    'FI_heading_prediction_day1','FI_heading_prediction_day2','FI_heading_prediction_day3', ...
    'FS_heading_prediction_day1','FS_heading_prediction_day2','FS_heading_prediction_day3'}, ...
    'RowNames', allanimals);

writetable(T, 'rate_predictions_bycellgroup.xlsx', 'WriteRowNames', 1);
