...
for a = 1:length(animals)
    for d = 1:3
        ...
        trialContext = unique([tbl.trialId, tbl.contextId], 'rows');
        trialId = trialContext(:,1);
        contextId = trialContext(:,2);
        ...
        for c = 1:length(cellnames)
            ...
            numC1Trials = sum(ctbl.contextId == 1);
            numC2Trials = sum(ctbl.contextId == 2);
            if length(cellTrials) >= minTrial && numC1Trials >= 1 && numC2Trials >= 1
        ...
        contextClass = categorical(contextId);
        mdl = fitcsvm(rv, contextClass, 'kernelfunction', 'linear', 'leaveout', 'on', 'verbose', 0);
        err = kfoldLoss(mdl);
        acc = 1 - err;
        cPred{a,d} = acc;
    end
