...
ficells = unique(ba.animalSessionCellName(ba.isStable == 1));
tbl = calmaps(ismember(calmaps.animalSessionCellName, ficells),:);

cPred = trainPredict(tbl, 1,2);

%% 
function cPred = trainPredict(calmaps, trainDay, predictDay)
...
for a = 1:length(animals)
    ...
    vtbl = atbl(ismember(atbl.dig, {'Corr', 'Geo'}) & atbl.dayUsed == trainDay,:);
    validTrials = unique(vtbl.trialId);
    ...
    if length(unique(digClass)) < 2
        fprintf('%s does not have enough digs\n', animals{a});
        continue
    end
    ...
    % predidct
    vtbl = atbl(ismember(atbl.dig, {'Corr', 'Geo'}) & atbl.dayUsed == predictDay,:);
    validTrials = unique(vtbl.trialId);
    digClass = cell(length(validTrials), 1);
    for t = 1:length(validTrials)
        iTrial = validTrials(t);
   ...
   % cmdl = compact(mdl);
    ...
    predictedDigs = predict(pmdl, co_angles);
